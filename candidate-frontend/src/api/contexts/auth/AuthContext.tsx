...
export const AuthProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState<boolean>(typeof Cookies.get(CookieKeys.CANDIDATE_JWT_TOKEN) !== 'undefined')
  const {
    isLoading,
    data: user,
    error
    ...
    onSuccess: (data) => {
      if (data.jwt) {
        Cookies.set(CookieKeys.CANDIDATE_JWT_TOKEN, data.jwt, { expires: 2 })
      }
    }
  })

  const onLoginSuccess = (jwt: string) => {
    Cookies.set(CookieKeys.CANDIDATE_JWT_TOKEN, jwt, { expires: 2 })
    setIsLoggedIn(true)
  ...
  const onLogout = (path: string = '/') => {
    Cookies.remove(CookieKeys.CANDIDATE_JWT_TOKEN)
    setIsLoggedIn(false)
    queryClient.invalidateQueries('currentUser')
